...
class PromptTemplate(BaseModel):
    ...
    template: str = Field(..., description="The prompt template string",
                          examples=["You are a helpful assistant. {message}", "Generate a response for: {message}"])
    input_variables: Dict[str, str] = Field(default_factory=dict, description="Input variables and their descriptions",
                                            examples=[{"message": "The user's input"},
                                                      {"message": "The user's input", "context": "Additional context"}])
    examples: Optional[Dict[str, str]] = Field(None, description="Example inputs and outputs",
...
class SongRecommendationPrompt(PromptTemplate):
    ...
    include_reason: bool = Field(default=True, description="Whether to include reason for each recommendation",
                                 examples=[True, False])
