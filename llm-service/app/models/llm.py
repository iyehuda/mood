...
class LLMServiceConfig(BaseModel):
    ...
    convert_system_message_to_human: bool = Field(default=True, description="Whether to convert system messages to human messages", examples=[True, False])

class LLMChainConfig(BaseModel):
    """Model for LLM chain configuration."""
    prompt_template: str = Field(..., description="The prompt template string", examples=["You are a helpful assistant. {message}", "Generate a response for: {message}"])
    input_variables: List[str] = Field(default=["message"], description="Input variables for the prompt template", examples=[["message"], ["message", "context"]])
    output_parser: Optional[Any] = Field(None, description="The output parser to use")

class LLMResponse(BaseModel):
    """Model for LLM response."""
    content: Union[str, Dict[str, Any]] = Field(..., description="The generated response content", examples=["This is a response", {"key": "value"}])
